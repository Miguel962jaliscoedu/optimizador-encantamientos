/**
 * Minecraft Enchantment Data
 * Contains all enchantment definitions, item mappings, and compatibility rules.
 */
...
export const ENCHANTMENTS = {
  ...
  mending: {
    ...
    items: [
      "helmet", "chestplate", "leggings", "boots", "pickaxe", "shovel",
      "axe", "sword", "hoe", "brush", "fishing_rod", "bow", "shears",
      "flint_and_steel", "carrot_on_a_stick", "warped_fungus_on_a_stick",
      "shield", "elytra", "trident", "turtle_shell", "crossbow", "mace", "spear"
    ]
  },
  multishot: {
    levelMax: 1,
  ...
  vanishing_curse: {
    ...
    items: [
      ...
      "axe", "sword", "hoe", "brush", "fishing_rod", "bow", "shears",
      "flint_and_steel", "carrot_on_a_stick", "warped_fungus_on_a_stick",
      "shield", "elytra", "pumpkin", "trident", "turtle_shell", "crossbow", "mace", "spear"
    ]
  },
  density: {
    levelMax: 5,
...
export const ITEMS = [
  ...
  // Tools
  "pickaxe", "shovel", "hoe", "shield", "brush",
  // Misc
  "fishing_rod", "shears", "flint_and_steel", "carrot_on_a_stick",
  "warped_fungus_on_a_stick", "pumpkin", "book"
];

export const ITEM_CATEGORIES = {
  armor: ["helmet", "chestplate", "leggings", "boots", "turtle_shell", "elytra"],
  weapons: ["sword", "axe", "mace", "spear"],
  ranged: ["trident", "bow", "crossbow"],
  tools: ["pickaxe", "shovel", "hoe", "shield", "brush"],
  misc: ["fishing_rod", "shears", "flint_and_steel", "carrot_on_a_stick", "warped_fungus_on_a_stick", "pumpkin", "book"]
};

/**
 * Get all enchantments compatible with a given item.
 */
export function getEnchantmentsForItem(itemNamespace) {
  if (itemNamespace === "book") {
    return Object.keys(ENCHANTMENTS);
  }
  return Object.keys(ENCHANTMENTS).filter(key =>
    ENCHANTMENTS[key].items.includes(itemNamespace)
  );
}

/**
 * Get the incompatible group for an enchantment.
 */
export function getIncompatibleGroup(enchantmentNamespace) {
  const queue = [enchantmentNamespace];
  const group = [];
  ...
  while (queue.length) {
    ...
    group.push(current);
    const meta = ENCHANTMENTS[current];
    if (meta) {
      meta.incompatible.forEach(inc => {
        if (!group.includes(inc) && !queue.includes(inc)) {
          queue.push(inc);
        }
      });
    }
...
/**
 * Get the maximum enchantment level across all enchantments.
 */
export function getMaxEnchantmentLevel() {
  return Math.max(...Object.values(ENCHANTMENTS).map(e => e.levelMax));
}
